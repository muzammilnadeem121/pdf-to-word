...
class ConversionService:
    ...
    def convert(self, pdf_path: str) -> dict:
        ...
        # Detect dominant font and use it in the DOCX
        dominant_font = self._layout.detect_dominant_font(pdf_path)
        logger.info("Dominant font detected: %s", dominant_font)
        exporter = WordExporter(font_name=dominant_font)

        output_filename = f"{uuid.uuid4()}.docx"
        output_path     = self._output_dir / output_filename
        
        if layout_blocks:
            exporter.export_layout(layout_blocks, str(output_path))
        else:
            exporter.export(extraction, str(output_path))

        # Delete the uploaded PDF — no longer needed
        try:
            os.remove(pdf_path)
            logger.info("Deleted uploaded file: %s", pdf_path)
        except Exception as exc:
            logger.warning("Could not delete upload %s: %s", pdf_path, exc)
